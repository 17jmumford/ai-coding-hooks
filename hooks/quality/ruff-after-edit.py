#!/usr/bin/env python3
"""Self-contained: post-edit ruff check. Requires Python 3.10+ and ruff or uv when used."""
...
def _ruff(file_path: Path, cwd: Path) -> _Proc:
    ruff = shutil.which("ruff")
    if ruff:
        return _run([ruff, "check", str(file_path)], cwd)
    uv = shutil.which("uv")
    if uv:
        return _run([uv, "run", "ruff", "check", str(file_path)], cwd)
    return _Proc(True, "", "", [])
...
def main() -> int:
    ...
    if res.ok or not res.cmd:
        _write_out(_allow_empty(host, data))
        return 0
    _write_out(_feedback(host, f"ruff check failed on {path}:\n{res.stderr or res.stdout}"))
    return 0
