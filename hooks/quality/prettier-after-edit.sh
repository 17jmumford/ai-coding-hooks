#!/usr/bin/env bash
# Self-contained: post-edit Prettier --check via npx. Requires: bash, jq, npx (Node).
set -euo pipefail

if ! command -v jq >/dev/null 2>&1; then
  echo >&2 "ai-coding-hooks: prettier-after-edit.sh requires jq"
  echo "{}"
  exit 0
fi
...
set +e
OUT=$(cd "$CWD" && npx --no-install prettier --check "$FILE" 2>&1)
XS=$?
set -e
...
MSG="prettier --check failed on ${FILE}:
${OUT}"
feedback "$MSG"
exit 0
